...
![Стартовый экран — загрузка файла или подключение к БД](screens/1.png)
...
![Шаблоны анализа — 6 готовых сценариев старта](screens/2.png)
...
![Схема данных — типы колонок и индикаторы размера](screens/7.png)
...
![Ответ на вопрос с автоматическим контекстом](screens/8.png)
...
![Просмотр сырых данных в модальном окне](screens/6.png)
...
![Отчёт о качестве данных — 81.9 из 100 с разбивкой проблем](screens/9.png)
...
![Дашборд с метриками по полу и таблицей корреляций](screens/3.png)
...
![Гистограммы выживаемости по классу/полу и размеру семьи](screens/4.png)
...
![История данных в формате нарратива с кольцевой диаграммой](screens/10.png)
...
![Итоговый портрет с топ-предикторами и рекомендациями](screens/5.png)
...
1. **Снапшот данных** — все загруженные таблицы копируются в CSV на момент
   создания. Дашборд не зависит от сессии и не сломается, если удалишь файл.
...
вместо «выгрузить в Excel, переделать в PowerBI, опубликовать, настроить доступы».
...
Качество системного промпта проверялось на собственном бенчмарке —
...
Если нужна приватность, добавь пароль при создании. Удалить дашборд можно
...
Через read-only роль на стороне БД — рекомендуемый путь. Connection strings
шифруются Fernet перед сохранением, доступны только владельцу и админу.
